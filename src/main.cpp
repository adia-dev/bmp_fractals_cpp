...
int main(int argc, char *argv[]) {
  ...
  {
    ScopeTimer _("fractal_generation");
    image.for_each_pixels([&](uint32_t x, uint32_t y) {
      double u = x / (double)height;
      double v = y / (double)height;

      auto [t, iterations] =
          /* Fractals::Mandelbrot::generate(2.5 * (u - 0.5) - 1.4, 2.5 * (v -
             0.5), max_iterations); */
          /* Fractals::Julia::generate(2.5 * (u - 0.5) - 1.4, 2.5 * (v - 0.5),
             max_iterations); */
          Fractals::BurningShip::generate(2.5 * (u - 0.5) - 1.4,
                                          2.5 * (v - 0.5), max_iterations);
      ...
      RGBColor c = Fractals::gradient_palette(iterations,
                                              RGBColor::blue_green_palette());
